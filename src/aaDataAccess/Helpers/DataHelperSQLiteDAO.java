...
public class DataHelperSQLiteDAO <T> implements aaIDAO<T> {
    protected final Class<T>DTOClass;
    protected final String  tableName;
    protected final String  tablePK;

    private static final String DBPath = aaAppConfig.getDATABASE(); 
     ...
     */
    public DataHelperSQLiteDAO(Class<T> dtoClass, String tableName, String tablePK) throws aaAppException {
        try {
            openConnection();
        } catch (SQLException e) {
            throw new aaAppException(null, e, getClass(), "DataHelperSQLiteDAO");
        }
    ...
    @Override
    public boolean create(T entity) throws aaAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            ...
            throw new aaAppException(null, e, getClass(), "create");
        }
    }

    @Override
    public boolean update(T entity) throws aaAppException {
        try {
            Field[] fields = DTOClass.getDeclaredFields();
            ...
            throw new aaAppException(null, e, getClass(), "update");
        }
    }

    @Override
    public boolean delete(Integer id) throws aaAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            ...
            throw new aaAppException(null, e, getClass(), "delete");
        }
    }

    @Override
    public T readBy(Integer id) throws aaAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            ...
            throw new aaAppException(null, e, getClass(), "readBy");
        }
    }

    @Override
    public List<T> readAll() throws aaAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
            ...
            throw new aaAppException(null, e, getClass(), "readAll");
        }
        return list;
    }

    @Override
    public Integer getMaxReg(String tableCelName) throws aaAppException {
        String sql = String.format("SELECT MAX(%s) FROM %s WHERE Estado = 'A'", tableCelName, tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
            throw new aaAppException(null, e, getClass(), "getMaxReg(...)");
        }
    }
    @Override
    public Integer getMinReg(String tableCelName) throws aaAppException {
        String sql = String.format("SELECT MIN(%s) FROM %s WHERE Estado = 'A'", tableCelName, tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
            throw new aaAppException(null, e, getClass(), "getMinReg(...)");
        }
    }
    @Override
    public Integer getCountReg() throws aaAppException {
        String sql = String.format("SELECT COUNT(*) FROM %s WHERE Estado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
            throw new aaAppException(null, e, getClass(), "getCountReg(...)");
        }
    }

    protected T mapResultSetToEntity(ResultSet rs) throws aaAppException {
        try {
            ...
            throw new aaAppException(null, e, getClass(), "mapResultSetToEntity");
        }
    }
}